# Copyright 2022-2026 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

EAPI=8

inherit go-env go-module systemd toolchain-funcs

GIT_REVISION=db8809540e1a7a9da5d518876894933ff55692ab

# Bumped whenever the patch set below changes, so that a node can be identified
# from `containerd --version` alone.
PATCHSET_ID=qgroup1

DESCRIPTION="A daemon to control runC"
HOMEPAGE="https://containerd.io/"
SRC_URI="https://github.com/containerd/containerd/archive/v${PV}.tar.gz -> ${P}.tar.gz"

LICENSE="Apache-2.0"
SLOT="0"
KEYWORDS="~amd64 ~arm64"
IUSE="apparmor btrfs device-mapper +cri +seccomp selinux test"
# tests require root or docker
RESTRICT="test"

# Compute btrfs snapshot usage from quota accounting (squota) instead of
# walking every snapshot on every stats pass.
# https://github.com/containerd/containerd/issues/13967
PATCHES=(
	"${FILESDIR}"/${PN}-${PV}-btrfs-qgroup-usage.patch
)

DEPEND="
	btrfs? ( sys-fs/btrfs-progs )
	seccomp? ( sys-libs/libseccomp )
"
# recommended minimum version of runc is found in script/setup/runc-version
RDEPEND="${DEPEND}
	>=app-containers/runc-1.4.3[apparmor?,seccomp?]
	selinux? ( sec-policy/selinux-docker )
"
BDEPEND="
	dev-go/go-md2man
	>=dev-lang/go-1.26.3
	virtual/pkgconfig
"

src_prepare() {
	default
	sed -i \
		-e "s/-s -w//" \
		Makefile || die
	sed -i \
		-e "s:/usr/local:/usr:" \
		containerd.service || die
}

src_compile() {
	local options=(
		$(usev apparmor)
		$(usex btrfs "" "no_btrfs")
		$(usex cri "" "no_cri")
		$(usex device-mapper "" "no_devmapper")
		$(usev seccomp)
		$(usev selinux)
	)

	local myemakeargs=(
		BUILDTAGS="${options[*]}"
		REVISION="${GIT_REVISION}+${PATCHSET_ID}"
		VERSION=v${PV}
	)

	emake "${myemakeargs[@]}" all

	# race condition in man target https://bugs.gentoo.org/765100
	tc-env_build go-env_run emake "${myemakeargs[@]}" man -j1 #nowarn
}

src_install() {
	rm bin/gen-manpages || die
	dobin bin/*
	doman man/*
	newconfd "${FILESDIR}"/${PN}.confd "${PN}"
	newinitd "${FILESDIR}"/${PN}.initd "${PN}"
	systemd_dounit containerd.service
	keepdir /var/lib/containerd

	# we already installed manpages, remove markdown source
	# before installing docs directory
	rm -r docs/man || die

	local DOCS=( ADOPTERS.md README.md RELEASES.md ROADMAP.md SCOPE.md docs/. )
	einstalldocs
}
